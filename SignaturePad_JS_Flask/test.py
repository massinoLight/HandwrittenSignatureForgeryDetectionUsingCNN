...
def on_text(text):
    ...
    def on_enter(cursor):
        print
        '%s: on_enter(%r)' % (name, cursor)

    @canvas.event
    def on_leave(cursor):
        print
        '%s: on_leave(%r)' % (name, cursor)

    @canvas.event
    def on_motion(cursor, x, y, pressure):
        print
        '%s: on_motion(%r, %r, %r, %r)' % (name, cursor, x, y, pressure)


@window.event
def on_mouse_press(x, y, button, modifiers):
    print
    'on_mouse_press(%r, %r, %r, %r' % (x, y, button, modifiers)


@window.event
def on_mouse_release(x, y, button, modifiers):
    print
    'on_mouse_release(%r, %r, %r, %r' % (x, y, button, modifiers)
